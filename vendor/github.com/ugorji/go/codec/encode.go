...
func (f *encFnInfo) kSlice(rv reflect.Value) {
	...
	if ti.rtid == uint8SliceTypId || rtelem.Kind() == reflect.Uint8 {
		...
		case seqTypeArray:
			...
			if rv.CanAddr() {
				...
				// TODO: Test that reflect.Copy works instead of manual one-by-one id:674 gh:675
				// for i := 0; i < l; i++ {
	...
	if l > 0 {
		...
		// TODO: Consider perf implication of encoding odd index values as symbols if type is string id:561 gh:562
		for j := 0; j < l; j++ {
			if cr != nil {
				if ti.mbs {
					if l%2 == 0 {
...
type Encoder struct {
	...
	e encDriver
	// NOTE: Encoder shouldn't call it's write methods, id:1048 gh:1049
...
// Examples:
//
//      // NOTE: 'json:' can be used as struct tag key, in place 'codec:' below. id:693 gh:694
